...
export class BannerScene extends Phaser.Scene {
  ...
  private readonly onBannerGoto = ({ index }: { index: number }) => {
    if (this.autoEnabled) this.resetAutoTimer()
    this.goto(index)
  }

  private readonly onBannerAuto = ({ enabled }: { enabled: boolean }) => {
    this.autoEnabled = enabled
    this.autoTimer.paused = !enabled
  }

  init(data: { startIndex?: number; singleMode?: boolean } = {}) {
    this.startIndex = data.startIndex ?? 0
    this.singleMode = data.singleMode ?? false
    if (this.singleMode) this.autoEnabled = false
  }
  ...
  create() {
    ...
    this.container.x = -width * this.current
    ...
    bus.on('banner:auto', this.onBannerAuto)

    // ── Auto-advance ─────────────────────────────────────────
    this.autoTimer = this.time.addEvent({
      delay: 4000,
      loop: true,
      paused: !this.autoEnabled,
  ...
  private buildSlide(
    ...
    if (this.singleMode && index !== this.current) {
      return this.add.container(index * w, 0)
    }
  ...
  shutdown() {
    ...
    bus.off('banner:auto', this.onBannerAuto)
  }
}
